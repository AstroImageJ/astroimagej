                ...
                if (detrendCount > 0) {
                    ...
                    if ((detrendVarsUsed[curve] > 0 || detrendFitIndex[curve] == 9 && useTransitFit[curve]) && detrendYNotConstant && detrendCount > (detrendFitIndex[curve] == 9 && useTransitFit[curve] ? 7 : 0) + detrendVarsUsed[curve] + 2) { //need enough data to provide degrees of freedom for successful fit
                        ...
                        if (detrendFitIndex[curve] == 9 && useTransitFit[curve]) {
                            ...
                            yModel1 = IJU.transitModel(xModel1, bestFit[0], bestFit[4], bestFit[1], bestFit[2], bestFit[3], orbitalPeriod[curve], forceCircularOrbit[curve] ? 0.0 : eccentricity[curve], forceCircularOrbit[curve] ? 0.0 : omega[curve], bestFit[5], bestFit[6], useLonAscNode[curve], lonAscNode[curve]).first();

                            yModel2 = IJU.transitModel(xModel2, bestFit[0], bestFit[4], bestFit[1], bestFit[2], bestFit[3], orbitalPeriod[curve], forceCircularOrbit[curve] ? 0.0 : eccentricity[curve], forceCircularOrbit[curve] ? 0.0 : omega[curve], bestFit[5], bestFit[6], useLonAscNode[curve], lonAscNode[curve]).first();
    ...
    public class FitLightCurveChi2 implements MinimizationFunction {
        ...
        public double function(double[] param) {
            ...
            if (useTransitFit[curve]) {
                ...
                var modelOut = IJU.transitModel(detrendX, f0, incl, p0, ar, tc, orbitalPeriod[curve], e, ohm, u1, u2, useLonAscNode[curve], lonAscNode[curve]);
                if (!modelOut.second()) {
                    AIJLogger.log("Failed to converge transit model for curve: " + curve);
                    return Double.POSITIVE_INFINITY;
                }
                lcModel = modelOut.first();
            }
