...
//	protected boolean wideTable=true;
    ...
    public static final Property<List<ShapedApertureRoi>> SHAPED_IMPORTED_APS =
            new Property<>(new ArrayList<>(),
                    MultiAperture_::serializeApertures, MultiAperture_::deserializeApertures,
                    MultiAperture_.class);
...
//        imp.getCanvas().requestFocusInWindow();
        ...
        if (runningWCSOnlyAlignment) {
            ...
            imp.getWindow().requestFocus();
            imp.getCanvas().requestFocusInWindow();
            if (apLoading.get().isPrevious() && useWCS && hasWCS &&
                    ((raPosStored != null && decPosStored != null) || apertureShape.get() == ApertureShape.ELLIPTICAL)) {
                enterPressed = true;
...
//            }
        ...
        if (apertureShape.get() == ApertureShape.ELLIPTICAL) {
            ...
            ApertureRoi selectedAp = null;
            if (!autoMode && !((apLoading.get().isPrevious() || previous) && firstClick) && ngot < nApertures) {
                apertureClicked = (selectedAp = ocanvas.findApertureRoi(xCenter, yCenter, 0)) != null;
            }

            if (autoMode) {
                shapedApertureRois.clear();
                var setting = apLoading.get() == ApLoading.IMPORTED ? SHAPED_IMPORTED_APS : SHAPED_APS;
                setting.reread();

                if (apLoading.get() == ApLoading.FIRST_PREVIOUS && !setting.get().isEmpty()) {
                    shapedApertureRois.add(setting.get().get(0));
                } else {
                    shapedApertureRois.addAll(setting.get());
                }

                if (shapedApertureRois.isEmpty()) {
                    shutDown();
                    return;
                }
                ...
                for (int i = 0; i < shapedApertureRois.size(); i++) {
                    ...
                    if ((apLoading.get().isPrevious() || previous) && (singleStep || !useWCS)) {
                        if (i == 0) {
                ...
                var d = showWarning("Loading apertures...");
                var warning = new AnnotateRoi(false, false, true, false, imp.getWidth() / 2f, imp.getHeight() / 2f, 2, "Loading apertures...", Color.GREEN);
            ...
            if (MOVING_T1.get() && (t1Path == null || !t1Path.isReady()) && !shapedApertureRois.isEmpty()) {
                if (enterPressed && t1Path == null) {
                    ...
                    hasWCS = asw.hasWCS();
                    var radec = hasWCS ?
                            wcs.pixels2wcs(new double[]{roi.getXpos(), roi.getYpos()}) :
                            new double[]{Double.NaN, Double.NaN};
                    t1Path = new Path(radec[0], radec[1], roi.getXpos(), roi.getYpos());
                ...
                if (enterPressed) {
                    ...
                    hasWCS = asw.hasWCS();
                    var radec = hasWCS ?
                            wcs.pixels2wcs(new double[]{movingTarget.getXpos(), movingTarget.getYpos()}) :
                            new double[]{Double.NaN, Double.NaN};

                    var tf = Path.getTime(imp);
                    imp.setSlice(firstSlice);
                    asw.updateWCS();
                    wcs = asw.getWCS();
                    hasWCS = asw.hasWCS();
                    var ts = Path.getTime(imp);

                    t1Path = t1Path.withTiming(ts, tf).withTarget(radec[0], radec[1], movingTarget.getXpos(), movingTarget.getYpos());
            ...
            if (enterPressed) {
                ...
                for (ShapedApertureRoi roi : shapedApertureRois) {
                    if (hasWCS) {
        ...
        if (enterPressed) {
            ...
            if (MOVING_T1.get() && (t1Path == null || !t1Path.isReady()) && ngot > 0) {
                if (t1Path == null) {
    ...
    private boolean automaticCompStarSelection(int currentRefStarCount) {
        ...
        var radius = switch (apertureShape.get()) {
            ...
            case ELLIPTICAL -> shapedApertureRois.get(referenceStar-1).getRadius();
        };
        ...
        final double t1Source = switch (apertureShape.get()) {
            ...
            case ELLIPTICAL -> {
                measurePhotometry(FitsJ.getHeader(imp), shapedApertureRois.get(referenceStar-1));
                yield  photom.sourceBrightness();
            }
        };
        ...
        final Pair.DoublePair t1coord = switch (apertureShape.get()) {
            ...
            case ELLIPTICAL -> {
                var ap = shapedApertureRois.get(referenceStar-1);
                yield new Pair.DoublePair(ap.getXpos(), ap.getYpos());
            }
        };
        ...
        if (!set.isEmpty()) {
            ...
            for (WeightedCoordinateMaxima coordinateMaxima :
                ...
                switch (apertureShape.get()) {
                    ...
                    case ELLIPTICAL -> {
                        try {
                            var t1 = shapedApertureRois.get(referenceStar-1);
    ...
    private TreeSet<StarFinder.CoordinateMaxima> removeCloseStars(TreeSet<StarFinder.CoordinateMaxima> initialSet, double t1Source, double maxP, double radius) {
        ...
        n = initialSet.parallelStream().map(m -> switch (apertureShape.get()) {
            ...
            case ELLIPTICAL -> {
                ...
                if (c.measure(imp, ap, true, backIsPlane, removeBackStars)) {
                    ap.moveTo(c.x() + dx, c.y() + dy, true);
                    ap.automaticTransform(c, true, !SHAPED_AP_ECCENTRICITY_LOCKED.get(), !SHAPED_AP_ANGLE_LOCKED.get());
                }

                var photom = measurePhotometry(imp, FitsJ.getHeader(imp), ap);

                for (ShapedApertureRoi a : shapedApertureRois) {
                    if (squaredDistanceTo(a.getXpos(), a.getYpos(), ap.getXpos(), ap.getYpos()) <= (radius2))
                        yield null;
                }
    ...
    public void mouseDragged(MouseEvent e) {
        ...
        if ((apertureShape.get() == ApertureShape.FREEFORM || apertureShape.get() == ApertureShape.ELLIPTICAL) &&
...
//                        }
                        ...
                        if (ap == 0 && MOVING_T1.get() && t1Path.isReady()) {
                            var l = t1Path.lerp(imp, hasWCS);
                            double[] xy = hasWCS && t1Path.wcsCapable() ?
                                    wcs.wcs2pixels(new double[]{l.first(), l.second()}) :
                                    new double[]{l.first(), l.second()};
                            xPos[ap] = xy[0];
                            yPos[ap] = xy[1];
                            xCenter = xy[0];
                            yCenter = xy[1];
...
//        IJ.log("setting up headings");
    ...
    protected GenericSwingDialog dialog() {
        ...
        gd.addNewSwappableSectionPanel(ApertureShape.class, (g, shape) -> {
            switch (shape) {
                ...
                case ELLIPTICAL -> {
                    ...
                    SHAPED_AP_ECCENTRICITY_LOCKED.addListener((k, b) -> {
                        setEnabled(ePanel, b);
                    });
                }
            }
        ...
        if (stackSize > 1) {
            ...
            var fs3 = GenericSwingDialog.getSpinnerFromSlider(sliders[7]).get();
            ...
            var ls3 = GenericSwingDialog.getSpinnerFromSlider(sliders[8]).get();
            ...
            fs3.addChangeListener(fcl);
            ls1.addChangeListener(lcl);
            ls2.addChangeListener(lcl);
            ls3.addChangeListener(lcl);
        }
            ...
            switch (shape) {
                case CIRCULAR, ELLIPTICAL -> {
                    ...
                    movingCB.setToolTipText("""
                            ...
                            Useful for asteroid or other moving object photometry<br>
...
//		}
    ...
    private static List<ShapedApertureRoi> deserializeApertures(String setting) {
    ...
    private static String serializeApertures(List<ShapedApertureRoi> apertures) {
    ...
    public enum ApLoading implements RadioEnum {
        ...
        private static int storedCount() {
            return switch (apertureShape.get()) {
                ...
                case ELLIPTICAL -> SHAPED_APS.get().size();
            };
        }
        ...
        private static int importedCount() {
            return switch (apertureShape.get()) {
                ...
                case ELLIPTICAL -> SHAPED_IMPORTED_APS.get().size();
            };
        }
    }
    ...
    public enum ApertureShape implements NState<ApertureShape>, ToolTipProvider {
        ...
        ELLIPTICAL("""
                <html>
                Elliptical apertures based on centroided results
                </html>
                """, "Elliptical"),
        FREEFORM("""
                <html>
    ...
    record Path(double ra0, double dec0, double x0, double y0, double ra1, double dec1, double x1, double y1, double start, double end) {
        Path(double ra0, double dec0, double x0, double y0) {
            this(ra0, dec0, x0, y0, Double.NaN, Double.NaN, Double.NaN, Double.NaN, Double.NaN, Double.NaN);
        }

        boolean hasTarget() {
            return (Double.isFinite(ra1) && Double.isFinite(dec1)) || (Double.isFinite(x1) && Double.isFinite(y1));
        }
        ...
        boolean wcsCapable() {
            return (Double.isFinite(ra1) && Double.isFinite(dec1)) && (Double.isFinite(ra0) && Double.isFinite(dec0));
        }

        Path withTarget(double ra1, double dec1, double x1, double y1) {
            return new Path(ra0, dec0, x0, y0, ra1, dec1, x1, y1, start, end);
        }

        Path withTiming(double start, double end) {
            return new Path(ra0, dec0, x0, y0, ra1, dec1, x1, y1, start, end);
        }
        ...
        Pair.DoublePair lerp(ImagePlus imp, boolean hasWCS) {
            if (!isReady()) {
                return hasWCS ? new Pair.DoublePair(ra0, ra1) : new Pair.DoublePair(x0, y0);
            }
        ...
        private Pair.DoublePair lerp(double t) {
            double x;
            double y;

            if (USE_FMA) {
                x = Math.fma(x1 - x0, t, x0);
                y = Math.fma(y1 - y0, t, y0);
            } else {
                x = (1 - t) * x0 + t * x1;
                y = (1 - t) * y0 + t * y1;
            }

            return new Pair.DoublePair(x, y);
        }

        public Pair.DoublePair slerp(ImagePlus imp) {
            if (!isReady()) {
        ...
        private Pair.DoublePair slerp(double t) {
