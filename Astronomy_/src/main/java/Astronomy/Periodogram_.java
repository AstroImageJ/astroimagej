...
public class Periodogram_ implements PlugIn {
    ...
    public void run(String arg) {
        ...
        String[] tableNames = astroj.MeasurementTable.getMeasurementTableNames();
        List<String> dataSourceList = new ArrayList<>();
        if (tableNames != null && tableNames.length > 0) {
            for (String t : tableNames) dataSourceList.add(t);
        }
        ...
        if (algorithm.equals("BLS")) {
            if (isMeasurementTable) {
                new Thread(() -> {
                    astroj.MeasurementTable table = astroj.MeasurementTable.getTable(dataSource);
                    if (table == null) {
                    ...
                    java.util.Arrays.sort(idx, java.util.Comparator.comparingDouble(i2 -> timeToSort[i2]));
                    for (int i = 0; i < n; i++) {
                    ...
                    class PeakResult {
                        int iteration;
                        int peakNumber;
                        double period, power, duration, depth, phase;
                        double t0_masking_model_center, t0_masking_min_avg;
                        PeakResult(int iteration, int peakNumber, double period, double power, double duration, double depth, double phase, double t0_masking_model_center, double t0_masking_min_avg) {
                            ...
                            this.depth = depth;
                            this.phase = phase;
                            this.t0_masking_model_center = t0_masking_model_center;
                            this.t0_masking_min_avg = t0_masking_min_avg;
                        }
                    }
                    List<PeakResult> allPeaks = new ArrayList<>();
                    double bjdOffset = Double.NaN;
                    try {
                        astroj.MeasurementTable measTable = astroj.MeasurementTable.getTable("Measurements");
                        int bjdCol = measTable.getColumnIndex("BJD_TDB");
                        if (bjdCol != astroj.MeasurementTable.COLUMN_NOT_FOUND && measTable.getCounter() > 0) {
                    ...
                    final java.util.concurrent.atomic.AtomicBoolean blsCancelled = new java.util.concurrent.atomic.AtomicBoolean(false);
                    java.awt.Button stopButton = new java.awt.Button("Stop");
                    ...
                    progressWin.add(buttonPanel, java.awt.BorderLayout.SOUTH);
                    progressWin.pack();
                    for (int planet = 0; planet < nPlanets; planet++) {
                        ...
                        for (int i = 0; i < nPeriods; i++) {
                            ...
                            try {
                                ...
                                if ((i + 1) % progressStep == 0 || i == nPeriods - 1) {
                                    ...
                                    ij.IJ.showProgress((i + 1) / (double) nPeriods);
                        ...
                        ij.IJ.showProgress(1.0); // Clear progress bar at end
                        System.out.println();
                        ...
                        if (bestPeriod > 0 && bestDuration > 0) {
                            ...
                            java.util.Arrays.sort(order, java.util.Comparator.comparingDouble(i -> sortedPhases[i]));
                            ...
                            phasePlot.setColor(java.awt.Color.RED);
                            ...
                            phasePlot.setColor(java.awt.Color.BLUE);
                            ...
                            phasePlot.setColor(java.awt.Color.BLACK);
                            phasePlot.show();
                        ...
                        if (bestIdx >= 0 && bestIdx < periods.length) {
                            ...
                            while (highlightLeft > 0 && power[highlightLeft] > highlightCutoff) highlightLeft--;
                            while (highlightRight < power.length - 1 && power[highlightRight] > highlightCutoff) highlightRight++;
                            ...
                            plot.setColor(java.awt.Color.RED); // Red
                            ...
                            plot.setColor(new java.awt.Color(255, 165, 0)); // Orange
                    ...
                    javax.swing.SwingUtilities.invokeLater(() -> {
                        ...
                        javax.swing.JTable blsTable = new javax.swing.JTable(data, columnNames);
                        blsTable.setAutoResizeMode(javax.swing.JTable.AUTO_RESIZE_OFF);
                        blsTable.setCellSelectionEnabled(true);
                        javax.swing.JScrollPane scrollPane = new javax.swing.JScrollPane(blsTable);
                        javax.swing.JButton saveButton = new javax.swing.JButton("Save as CSV");
                        saveButton.addActionListener(e -> {
                            javax.swing.JFileChooser fileChooser = new javax.swing.JFileChooser();
                            fileChooser.setFileFilter(new javax.swing.filechooser.FileNameExtensionFilter("CSV Files", "csv"));
                            if (fileChooser.showSaveDialog(blsTable) == javax.swing.JFileChooser.APPROVE_OPTION) {
                                ...
                                try (java.io.PrintWriter pw = new java.io.PrintWriter(new java.io.OutputStreamWriter(new java.io.FileOutputStream(file), java.nio.charset.StandardCharsets.UTF_8))) {
                                    ...
                                    javax.swing.JOptionPane.showMessageDialog(blsTable, "Error saving file: " + ex.getMessage());
                                }
                            }
                        });
                        javax.swing.JPanel panel = new javax.swing.JPanel(new java.awt.BorderLayout());
                        panel.add(scrollPane, java.awt.BorderLayout.CENTER);
                        panel.add(saveButton, java.awt.BorderLayout.SOUTH);
                        javax.swing.JFrame frame = new javax.swing.JFrame("BLS Multi-Planet Results");
                        frame.setDefaultCloseOperation(javax.swing.JFrame.DISPOSE_ON_CLOSE);
                        frame.setContentPane(panel);
                        // Auto-resize columns to fit content first
                        javax.swing.table.TableColumnModel blsColumnModel = blsTable.getColumnModel();
                        for (int col = 0; col < blsTable.getColumnCount(); col++) {
                            int width = 75; // min width
                            for (int row = 0; row < blsTable.getRowCount(); row++) {
                                java.awt.Component comp = blsTable.getCellRenderer(row, col).getTableCellRendererComponent(blsTable, blsTable.getValueAt(row, col), false, false, row, col);
                        ...
                        if (blsTable.getColumnCount() > 0) {
                            ...
                            javax.swing.table.TableColumn col = blsColumnModel.getColumn(lastCol);
                            javax.swing.table.JTableHeader headerComp = blsTable.getTableHeader();
                            java.awt.Component headerRenderer = col.getHeaderRenderer() != null ?
                            ...
                            for (int row = 0; row < blsTable.getRowCount(); row++) {
                                java.awt.Component comp = blsTable.getCellRenderer(row, lastCol).getTableCellRendererComponent(blsTable, blsTable.getValueAt(row, lastCol), false, false, row, lastCol);
                        ...
                        java.awt.Dimension tablePref = blsTable.getPreferredSize();
                        java.awt.Dimension scrollPanePref = scrollPane.getPreferredSize();
                        ...
                        frame.setMinimumSize(new java.awt.Dimension(600, 300));
                        try {
                            frame.setIconImage(ij.IJ.getInstance().getIconImage());
                        } catch (Exception ex) {
        ...
        if (algorithm.equals("Lomb-Scargle")) {
            ...
            if (dataSource.equals("Current Image")) {
                ...
                class LSResult {
                    int iteration;
                    int peakNumber;
                    double period, power, maskCenter;
                    LSResult(int iteration, int peakNumber, double period, double power, double maskCenter) {
                        this.iteration = iteration;
                        this.peakNumber = peakNumber;
                        this.period = period;
                        this.power = power;
                ...
                final java.util.concurrent.atomic.AtomicBoolean lsCancelled = new java.util.concurrent.atomic.AtomicBoolean(false);
                java.awt.Button lsStopButton = new java.awt.Button("Stop");
                lsStopButton.addActionListener(e -> {
                    lsCancelled.set(true);
                    progressWin.getTextPanel().append("\nLomb-Scargle run cancelled by user.\n");
                });
                java.awt.Panel lsButtonPanel = new java.awt.Panel();
                lsButtonPanel.add(lsStopButton);
                progressWin.add(lsButtonPanel, java.awt.BorderLayout.SOUTH);
                progressWin.pack();
                for (int planet = 0; planet < nPlanets; planet++) {
                ...
                javax.swing.SwingUtilities.invokeLater(() -> {
                    ...
                    String[][] data = rowList.toArray(new String[0][]);
                    javax.swing.JTable lsTable = new javax.swing.JTable(data, columnNames);
                    lsTable.setAutoResizeMode(javax.swing.JTable.AUTO_RESIZE_OFF);
                    lsTable.setCellSelectionEnabled(true);
                    javax.swing.JScrollPane scrollPane = new javax.swing.JScrollPane(lsTable);
                    javax.swing.JButton saveButton = new javax.swing.JButton("Save as CSV");
                    saveButton.addActionListener(e -> {
                        javax.swing.JFileChooser fileChooser = new javax.swing.JFileChooser();
                        fileChooser.setFileFilter(new javax.swing.filechooser.FileNameExtensionFilter("CSV Files", "csv"));
                        if (fileChooser.showSaveDialog(lsTable) == javax.swing.JFileChooser.APPROVE_OPTION) {
                            java.io.File file = fileChooser.getSelectedFile();
                            String fname = file.getName().toLowerCase();
                            if (!fname.endsWith(".csv")) {
                                file = new java.io.File(file.getParentFile(), file.getName() + ".csv");
                            ...
                            try (java.io.PrintWriter pw = new java.io.PrintWriter(new java.io.OutputStreamWriter(new java.io.FileOutputStream(file), java.nio.charset.StandardCharsets.UTF_8))) {
                                ...
                                javax.swing.JOptionPane.showMessageDialog(lsTable, "Error saving file: " + ex.getMessage());
                            }
                        }
                    });
                    javax.swing.JPanel panel = new javax.swing.JPanel(new java.awt.BorderLayout());
                    panel.add(scrollPane, java.awt.BorderLayout.CENTER);
                    panel.add(saveButton, java.awt.BorderLayout.SOUTH);
                    javax.swing.JFrame frame = new javax.swing.JFrame("Lomb-Scargle Multi-Planet Results");
                    frame.setDefaultCloseOperation(javax.swing.JFrame.DISPOSE_ON_CLOSE);
                    frame.setContentPane(panel);
                    // Auto-resize columns to fit content first
                    javax.swing.table.TableColumnModel lsColumnModel = lsTable.getColumnModel();
                    for (int col = 0; col < lsTable.getColumnCount(); col++) {
                        int width = 75;
                        for (int row = 0; row < lsTable.getRowCount(); row++) {
                            java.awt.Component comp = lsTable.getCellRenderer(row, col).getTableCellRendererComponent(lsTable, lsTable.getValueAt(row, col), false, false, row, col);
                            width = Math.max(comp.getPreferredSize().width + 10, width);
                        }
                    ...
                    if (lsTable.getColumnCount() > 0) {
                        ...
                        javax.swing.table.TableColumn col = lsColumnModel.getColumn(lastCol);
                        javax.swing.table.JTableHeader headerComp = lsTable.getTableHeader();
                        java.awt.Component headerRenderer = col.getHeaderRenderer() != null ?
                        ...
                        int maxCellWidth = headerWidth;
                        for (int row = 0; row < lsTable.getRowCount(); row++) {
                            java.awt.Component comp = lsTable.getCellRenderer(row, lastCol).getTableCellRendererComponent(lsTable, lsTable.getValueAt(row, lastCol), false, false, row, lastCol);
                            maxCellWidth = Math.max(maxCellWidth, comp.getPreferredSize().width + 10);
                        }
                        col.setPreferredWidth(maxCellWidth);
                    }
                    ...
                    // Ensure the frame is wide enough for all columns
                    java.awt.Dimension tablePref = lsTable.getPreferredSize();
                    java.awt.Dimension scrollPanePref = scrollPane.getPreferredSize();
                    int requiredWidth = Math.max(tablePref.width, scrollPanePref.width) + 20; // Add small padding
                    int requiredHeight = scrollPanePref.height + 80; // Add space for save button
            ...
            if (isMeasurementTable) {
                new Thread(() -> {
                    ...
                    class TLSResult {
                        int iteration;
                        int peakNumber;
                        double period, sde, duration, depth;
                        double t0_sliding, t0_minavg;
                        TLSResult(int iteration, int peakNumber, double period, double sde, double duration, double depth, double t0_sliding, double t0_minavg) {
                            this.iteration = iteration;
                            this.peakNumber = peakNumber;
                            this.period = period;
                            this.sde = sde;
                    ...
                    final java.util.concurrent.atomic.AtomicBoolean tlsCancelled = new java.util.concurrent.atomic.AtomicBoolean(false);
                    java.awt.Button tlsStopButton = new java.awt.Button("Stop");
                    tlsStopButton.addActionListener(e -> {
                        tlsCancelled.set(true);
                        tlsProgressWin.getTextPanel().append("\nTLS run cancelled by user.\n");
                    });
                    java.awt.Panel tlsButtonPanel = new java.awt.Panel();
                    tlsButtonPanel.add(tlsStopButton);
                    tlsProgressWin.add(tlsButtonPanel, java.awt.BorderLayout.SOUTH);
                    tlsProgressWin.pack();
                    for (int planet = 0; planet < nPlanets; planet++) {
                        ...
                        TLS.Result tlsResult = TLS.search(t, f, minPeriod, maxPeriod, nPeriods, minFracDur, maxFracDur, nDurations, u1, u2, (pi) -> {
                            ...
                            System.out.print("\r" + bar.toString());
                            // Update ImageJ progress bar
                            ij.IJ.showProgress((pi + 1) / (double) nPeriods);
                            // Update only the last line for this planet
                        ...
                        if (bestIdx >= 0 && bestIdx < tlsResult.periods.length) {
                            ...
                            int highlightLeft = bestIdx, highlightRight = bestIdx;
                            while (highlightLeft > 0 && tlsResult.sde[highlightLeft] > highlightCutoff) highlightLeft--;
                            while (highlightRight < tlsResult.sde.length - 1 && tlsResult.sde[highlightRight] > highlightCutoff) highlightRight++;
                    ...
                    javax.swing.SwingUtilities.invokeLater(() -> {
                        ...
                        String[][] data = rowList.toArray(new String[0][]);
                        javax.swing.JTable tlsTable = new javax.swing.JTable(data, columnNames);
                        tlsTable.setAutoResizeMode(javax.swing.JTable.AUTO_RESIZE_OFF);
                        tlsTable.setCellSelectionEnabled(true);
                        javax.swing.JScrollPane scrollPane = new javax.swing.JScrollPane(tlsTable);
                        javax.swing.JButton saveButton = new javax.swing.JButton("Save as CSV");
                        saveButton.addActionListener(e -> {
                            javax.swing.JFileChooser fileChooser = new javax.swing.JFileChooser();
                            fileChooser.setFileFilter(new javax.swing.filechooser.FileNameExtensionFilter("CSV Files", "csv"));
                            if (fileChooser.showSaveDialog(tlsTable) == javax.swing.JFileChooser.APPROVE_OPTION) {
                                java.io.File file = fileChooser.getSelectedFile();
                                String fname = file.getName().toLowerCase();
                                if (!fname.endsWith(".csv")) {
                                    file = new java.io.File(file.getParentFile(), file.getName() + ".csv");
                                ...
                                try (java.io.PrintWriter pw = new java.io.PrintWriter(new java.io.OutputStreamWriter(new java.io.FileOutputStream(file), java.nio.charset.StandardCharsets.UTF_8))) {
                                    ...
                                    javax.swing.JOptionPane.showMessageDialog(tlsTable, "Error saving file: " + ex.getMessage());
                                }
                            }
                        });
                        javax.swing.JPanel panel = new javax.swing.JPanel(new java.awt.BorderLayout());
                        panel.add(scrollPane, java.awt.BorderLayout.CENTER);
                        panel.add(saveButton, java.awt.BorderLayout.SOUTH);
                        javax.swing.JFrame frame = new javax.swing.JFrame("TLS Multi-Planet Results");
                        frame.setDefaultCloseOperation(javax.swing.JFrame.DISPOSE_ON_CLOSE);
                        frame.setContentPane(panel);
                        // Auto-resize columns to fit content first
                        javax.swing.table.TableColumnModel tlsColumnModel = tlsTable.getColumnModel();
                        for (int col = 0; col < tlsTable.getColumnCount(); col++) {
                            int width = 75; // min width
                            for (int row = 0; row < tlsTable.getRowCount(); row++) {
                                java.awt.Component comp = tlsTable.getCellRenderer(row, col).getTableCellRendererComponent(tlsTable, tlsTable.getValueAt(row, col), false, false, row, col);
                                width = Math.max(comp.getPreferredSize().width + 10, width);
                            }
                        ...
                        if (tlsTable.getColumnCount() > 0) {
                            ...
                            javax.swing.table.TableColumn col = tlsColumnModel.getColumn(lastCol);
                            javax.swing.table.JTableHeader headerComp = tlsTable.getTableHeader();
                            java.awt.Component headerRenderer = col.getHeaderRenderer() != null ?
                            ...
                            int maxCellWidth = headerWidth;
                            for (int row = 0; row < tlsTable.getRowCount(); row++) {
                                java.awt.Component comp = tlsTable.getCellRenderer(row, lastCol).getTableCellRendererComponent(tlsTable, tlsTable.getValueAt(row, lastCol), false, false, row, lastCol);
                                maxCellWidth = Math.max(maxCellWidth, comp.getPreferredSize().width + 10);
                            }
                            col.setPreferredWidth(maxCellWidth);
                        }
                        ...
                        // Ensure the frame is wide enough for all columns
                        java.awt.Dimension tablePref = tlsTable.getPreferredSize();
                        java.awt.Dimension scrollPanePref = scrollPane.getPreferredSize();
                        int requiredWidth = Math.max(tablePref.width, scrollPanePref.width) + 20; // Add small padding
                        int requiredHeight = scrollPanePref.height + 80; // Add space for save button
