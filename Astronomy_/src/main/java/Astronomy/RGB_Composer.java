package Astronomy;// RGB_Composer.java
...
public class RGB_Composer implements ActionListener, PlugIn
	...
	String[] getImages()
		{
		int nduds=0;
		String[] str = IJU.listOfOpenImages(null);
		if (str.length == 0)
			{
	...
	void auto (int color)
		...
		txt1.setText(gf.format(mn));
		txt2.setText(gf.format(mx));
		}
	...
	void contrastBrightness (JTextField mn, JTextField mx, int color, double fmn, double fmx)
		...
		mn.setText(gf.format(mnmx[0]));
		mx.setText(gf.format(mnmx[1]));
		frame.setVisible(true);
		compose(color);
		rgb.updateAndDraw();
		return;
