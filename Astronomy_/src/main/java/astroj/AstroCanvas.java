...
public class AstroCanvas extends OverlayCanvas {
        ...
        private Color dirIndicatorColorWCS = new Color(255,190,0);//(255,129,35);
...
@Override
	 public void drawZoomIndicator(Graphics g) {
        ...
        if (showYScale)
            ...
            ((Graphics2D)g).translate(-getWidth()/2.0, -getHeight()/2.0);
            ((Graphics2D)g).setColor(wcs.hasScale ? dirIndicatorColorWCS : dirIndicatorColor);
            ((Graphics2D)g).setFont (p16);
            int length = 160;
            int startX = (getWidth() - length)/2;
            int endX = startX + length;
