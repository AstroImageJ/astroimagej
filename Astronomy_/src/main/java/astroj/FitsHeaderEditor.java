...
//		frame.add (new JLabel("Image: "+imp.getShortTitle()));
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (cmd.equals(CANCEL)) {
            ...
            StringBuilder hdrString = new StringBuilder();
            for (int i = 0; i < hdr.length; i++) {
                hdrString.append(hdr[i]);
                hdrString.append(NL);
            }
    ...
    protected String[] extractHeader() {
        ...
        for (int i = 0; i < l; i++) {
            if (header[i][1] != null && (header[i][2] != null || header[i][4].trim().equals("H") ||
                ...
                key = new StringBuilder(header[i][1].trim());
                /*if (key.length() > 8)
                    {
                ...
                val = new StringBuilder(header[i][2] != null ? header[i][2].trim() : "");
                ...
                if (val.length() < 67 && header[i][3] != null) {
                    ...
                    while ((val.length() + comment.length()) < 70) {
                        val.append(" ");
                    }
                } else {
                    while ((val.length()) < 70) {
...
//        hdr = FitsJ.addHistory("Header modified by AstroIJ FITS editor, Version "+VERSION,hdr);
    ...
    class FITSTableModel extends AbstractTableModel {
        ...
        String comment;
        String type;
        private final String[] columnNames = {"#", "Keyword", "Value", "Comment", "Type"};
            ...
            if (newvalue instanceof String) {
                ...
                if (col == 1) {
                    if (keyword.equalsIgnoreCase("END")) {
                        ...
                        keyword = keyword.substring(0, Math.min(keyword.length(), 8)).toUpperCase();//todo test
                }
                getType();
