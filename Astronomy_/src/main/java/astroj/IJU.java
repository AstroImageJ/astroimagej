...
public class IJU
    ...
    public static String spectralTypeFromDensity(double d)
        ...
        for (int i=0; i<n-2; i++)
            {
            if (d>=(rhoStar[i]+rhoStar[i+1])/2 && d<(rhoStar[i+1]+rhoStar[i+2])/2) return spType[i+1]; 
            }
        return "error";
        }
    ...
    public static boolean backupAllAIJSettings(boolean fromImage)
        ...
        if (fromImage)
            ...
            if (!IJ.showMessageWithCancel("Close AIJ Images and Control Panels", "All AIJ Images and control panels, except the AIJ Toolbar and Multi-plot Panels,\n"+
                                                                "must be closed to enable capture of all current settings into the backup file.\n\n"+
                                                                "Press CANCEL to abort the backup, or press OK to continue."))
                {
                return false;
    ...
    public static void openRaDecApertures(String path)
        ...
        if (apsPath ==null || apsPath.equals(""))
            {
            IJ.beep();
...
//                            names.add(columns.length > 2 ? columns[2].trim() : "");
                            isRefs.add(columns.length > 2 ? 
                                      (columns[2].trim().equals("0") ? false : (columns[2].trim().equals("1") ? true : (ras.size() > 1 ? true : false))) :
                                      (ras.size() > 1 ? true : false));
                            isCentroids.add(columns.length > 3 ? (columns[3].trim().equals("0") ? false : true) : true);
    ...
    public static void showInSIMBAD(double ra, double dec, double searchRadius)
        ...
        try {
            ...
            object = URLEncoder.encode(decToSex(ra/15.0, 3, 24, false) + decToSex(dec, 2, 90, true),"UTF-8");
            ...
            if (compress)
                ...
                if (infile.exists())
                    ...
                    try {
                        ...
                        out.close();
                        infile.delete();
                        if (showLog) log("Compressed and saved "+logFilenameDescription+"file \""+savePath+"\"", showLogDateTime);
                        }
                    catch (IOException e)
                        {
