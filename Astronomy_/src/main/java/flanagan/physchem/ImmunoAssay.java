...
*   MODIFIED:   17-28 February 2011, 1 March 2011, 11 March 2011, 30 March 2011, 1 April 2012
*
*   DOCUMENTATION:
...
public class ImmunoAssay extends Regression{
    ...
    public ImmunoAssay(double[] analytes, double[] responses, double[] yWeights){
        this.titleZero = "Program ImmunoAssay";
        this.nAnalyteConcns = analytes.length;
        this.analyteConcns = Conv.copy(analytes);
        this.analyteConcnFlag = 0;
        ...
        super.yErrors = super.checkForZeroWeights(yWeights);
    ...
    // Constructor without title
    // with total analyte concns, arbitrary unit responses and both analyte and response errors entered
    public ImmunoAssay(double[] analytes, double[] responses, double[] xWeights, double[] yWeights){
        this.titleZero = "Program ImmunoAssay";
        this.nAnalyteConcns = analytes.length;
        this.analyteConcns = Conv.copy(analytes);
        this.analyteConcnFlag = 0;
        ...
        this.responsesEntered = true;
        this.nWeights = yWeights.length;
        super.yErrors = yWeights;
        double[][] xErrors = new double[1][this.nWeights];
        xErrors[0] = xWeights;
        super.xErrors = xErrors;
        super.jointZeroCheck(xErrors, yWeights);
        if(super.weightOpt)super.weightFlag = 1;
    ...
    // Constructor without title
    // with total analyte concns, arbitrary unit responses and weights entered
    public ImmunoAssay(double[] analytes, double[] responses, double[] yWeights, String title){
        this.titleZero = "Program ImmunoAssay: " + title;
        this.nAnalyteConcns = analytes.length;
        this.analyteConcns = Conv.copy(analytes);
        this.analyteConcnFlag = 0;
    ...
    // Constructor with title
    // with total analyte concns, arbitrary unit responses and yWeights entered
    public ImmunoAssay(double[] analytes, double[] responses, double[] xWeights, double[] yWeights, String title){
        this.titleZero = "Program ImmunoAssay: " + title;
        this.nAnalyteConcns = analytes.length;
        this.analyteConcns = Conv.copy(analytes);
        this.analyteEntered = true;
    ...
    public void enterWeights(double[] yWeights){
        this.setDataOneDone = false;
        this.nWeights = yWeights.length;
        super.yErrors = super.checkForZeroWeights(yWeights);
        super.yErrorsEntered = true;
    ...
    public void enterWeightsAslog10(double[] yWeights){
        this.setDataOneDone = false;
        this.nWeights = yWeights.length;
        super.yErrors = this.antiLog10(super.checkForZeroWeights(yWeights));
        if(super.weightOpt)super.weightFlag = 1;
        super.yErrorsEntered = true;
    }

    // Enter weighting factors as loge
    public void enterWeightsAslogE(double[] yWeights){
        this.setDataOneDone = false;
        this.nWeights = yWeights.length;
        super.yErrors = this.antiLoge(super.checkForZeroWeights(yWeights));
        if(super.weightOpt)super.weightFlag = 1;
        super.yErrorsEntered = true;
    }

    // Convert multiplicative weighting factors to uncertainty estimate yWeights
    public void enterMultiplicativeWeights(double[] yWeights){
        this.setDataOneDone = false;
        this.nWeights = yWeights.length;
        super.yErrors = Conv.copy(super.checkForZeroWeights(yWeights));
        for(int i=0; i<this.nWeights; i++)super.yErrors[i] = 1.0/Math.abs(super.yErrors[i]);
        if(super.weightOpt)super.weightFlag = 1;
        super.yErrorsEntered = true;;
    }

    // Convert multiplicative weighting factors to uncertainty estimate yWeights as log10
    public void enterMultiplicativeWeightsAsLog10(double[] yWeights){
        this.setDataOneDone = false;
        this.nWeights = yWeights.length;
        super.yErrors = this.antiLog10(super.checkForZeroWeights(Conv.copy(yWeights)));
        for(int i=0; i<this.nWeights; i++)super.yErrors[i] = 1.0/Math.abs(super.yErrors[i]);
        if(super.weightOpt)super.weightFlag = 1;
        super.yErrorsEntered = true;
    }

    // Convert multiplicative weighting factors to uncertainty estimate yWeights as loge
    public void enterMultiplicativeWeightsAsLogE(double[] yWeights){
        this.setDataOneDone = false;
        this.nWeights = yWeights.length;
        super.yErrors = this.antiLoge(super.checkForZeroWeights(Conv.copy(yWeights)));
        for(int i=0; i<this.nWeights; i++)super.yErrors[i] = 1.0/Math.abs(super.yErrors[i]);
        if(super.weightOpt)super.weightFlag = 1;
        super.yErrorsEntered = true;
    }
            ...
            if(pos1==-1){
                ...
                if(pos2!=-1){
                    ...
                    if(wL>0){
                        ...
                        super.yErrors[i] = Double.parseDouble(word);
                        if(super.yErrors[i]==1.0)weightCheck++;
                     }
                }
            }
        }
    ...
    private void fitAmersham(double[] estimates){
        ...
        if(super.xErrorsEntered){
            AmershamDual am1 = new AmershamDual();
            am1.setYerrors(super.yErrors);
            am1.setXerrors(super.xErrors);
            // Perform non-linear regression
            super.simplex(am1, start, step);
            super.removeConstraints();
            // Calculate responses for fitted curve
            for(int i=0; i<this.nInterp; i++){
                double[] xx = {this.interpolationConcns[i]};
                this.calculatedResponses[i] = am1.function(super.best, xx, -1)[0];
            }
        }
        else{
            Amersham am2 = new Amersham();
            // Perform non-linear regression
            super.simplex(am2, start, step);
            super.removeConstraints();
            // Calculate responses for fitted curve
            for(int i=0; i<this.nInterp; i++){
                double[] xx = {this.interpolationConcns[i]};
                this.calculatedResponses[i] = am2.function(super.best, xx);
            }
        }
    ...
    private void setDataOne(){
        ...
        if(super.yErrorsEntered){
            ...
            for(int i=0; i<this.nWeights; i++)super.yErrors[i] = 1.0;
        }
        ...
        if(super.yErrorsEntered){
            hold = Conv.copy(super.yErrors);
            for(int i=0; i<this.nAnalyteConcns; i++)super.yErrors[i] = hold[indices[i]];
        }
        if(super.xErrorsEntered){
            hold = Conv.copy(super.xErrors[0]);
            for(int i=0; i<this.nAnalyteConcns; i++)super.xErrors[0][i] = hold[indices[i]];
        }
        ...
        for(int i=0; i<this.nAnalyteConcns-1; i++){
            ...
            if(nIdent>1){
                ...
                double sumYW = 0.0;
                double sumXW = 0.0;
                for(int j=iFirst; j<iFirst+nIdent; j++){
                    sumR += this.responses[j];
                    if(super.yErrorsEntered)sumYW += super.yErrors[i]*super.yErrors[i];
                    if(super.xErrorsEntered)sumXW += super.xErrors[0][i]*super.xErrors[0][i];
                }
                this.responses[iFirst] = sumR/nIdent;
                if(super.yErrorsEntered)super.yErrors[iFirst] = Math.sqrt(sumYW)/nIdent;
                if(super.xErrorsEntered)super.xErrors[0][iFirst] = Math.sqrt(sumXW)/nIdent;
                for(int j=iFirst+1; j<this.nAnalyteConcns-nIdent+1; j++){
                    ...
                    if(super.yErrorsEntered)super.yErrors[j] = super.yErrors[j+nIdent-1];
                    if(super.xErrorsEntered)super.xErrors[0][j] = super.xErrors[0][j+nIdent-1];
        ...
        if(this.nAnalyteConcns<nHold){
            double[] holdd = this.analyteConcns;
            this.analyteConcns = new double[this.nAnalyteConcns];
            for(int i=0; i<this.nAnalyteConcns; i++)this.analyteConcns[i] = holdd[i];
            holdd = this.responses;
            this.responses = new double[this.nAnalyteConcns];
            for(int i=0; i<this.nAnalyteConcns; i++)this.responses[i] = holdd[i];
            holdd = super.yErrors;
            super.yErrors = new double[this.nAnalyteConcns];
            for(int i=0; i<this.nAnalyteConcns; i++)this.weight[i] = holdd[i];
            if(super.xErrorsEntered){
                holdd = super.xErrors[0];
                super.xErrors = new double[1][this.nAnalyteConcns];
                for(int i=0; i<this.nAnalyteConcns; i++)super.xErrors[0][i] = holdd[i];
            }
            if(this.analyteConcnFlag==1){
                holdd = this.log10AnalyteConcns;
                this.log10AnalyteConcns = new double[this.nAnalyteConcns];
                for(int i=0; i<this.nAnalyteConcns; i++)this.log10AnalyteConcns[i] = holdd[i];
            }
            if(this.analyteConcnFlag==2){
                holdd = this.logeAnalyteConcns;
                this.logeAnalyteConcns = new double[this.nAnalyteConcns];
                for(int i=0; i<this.nAnalyteConcns; i++)this.logeAnalyteConcns[i] = holdd[i];
            }
        }

        // log calculations for plotting
        ...
        // check for response values <= 0 for log plotting
        int nNonPos = 0;
        ...
        if(super.xErrorsEntered){
            super.setDefaultValues(xData, this.responses, super.xErrors, super.yErrors);
        }
        else{
            super.setDefaultValues(xData, this.responses, super.yErrors);
        }
    ...
    private void curveCheck(int mFlag){
        ...
        if(this.sampleErrorFlag){
            if(!super.yErrorsEntered){
                ...
                for(int i=0; i<nwe; i++)ew[i] = super.yErrors[i+iS];
                monoCheck = ImmunoAssay.isMonotonic(ew);

                // check for big jumps in the yWeights
                if(monoCheck){
                ...
                for(int i=0; i<this.nAnalyteConcns; i++)sWeights[i] = super.yErrors[i]*scale;
                if(super.yErrorsEntered && !monoCheck){
...
class AmershamDual implements RegressionFunction3{
    ...
    public double[] function(double[] p, double[] x, int ii){
        ...
        y[0] = this.calcResponse(p[0], p[1],  p[2], p[3], x[0], p[4]);
        if(ii>=0)y[1] = this.calcSquareWeight(p[0], p[1],  p[2], p[3], x[0], p[4], this.xErrors[0][ii], this.yErrors[ii]);
        return y;
    }
    ...
    public double calcResponse(double assocK, double labelF,  double antibodyF,  double nsbF, double analyteF, double scaleF){
       ...
       // Calculate response
    public double calcSquareWeight(double assocK, double labelF,  double antibodyF,  double nsbF, double analyteF, double scaleF, double xError, double yError){
        double term0 = 2.0*antibodyF*(labelF - nsbF)/labelF;
        double term1 = assocK + labelF + analyteF;
        double term2 = Math.sqrt(Fmath.square(term1 - antibodyF) + 4.0*assocK*antibodyF);
        double term3 = 1.0 + (term1 - antibodyF)/term2;
        double numer = scaleF*term0*term3;      
        double denom = term1 + antibodyF + term2;
        denom *= denom;
        double deriv = numer/denom;
        return yError*yError + deriv*deriv*xError*xError;
    }
