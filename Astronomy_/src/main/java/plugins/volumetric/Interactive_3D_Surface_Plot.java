...
import plugins.volumetric.jrenderer3d.JRenderer3D;
import plugins.volumetric.jrenderer3d.Line3D;
import plugins.volumetric.jrenderer3d.Text3D;
    ...
    private void readPrefs() {
        if (isExamplePlot) {  	
            ...
            if (frame != null) {
				...
				xloc = (screenWidth-windowWidth)/2;
				 yloc = (screenHeight-windowHeight)/2;
			}
        	light = 0.2;
        	perspective = 0.1;
        	grid = 256;
...
private JSlider createSliderHorizontal(String borderTitle, int min, int max, int value) {
	...
	private void setSliderTitle(JSlider slider, Color color, String str) {
		...
		Font sliderFont = new Font("Sans", Font.PLAIN, (int) (11 * Prefs.getGuiScale()));
