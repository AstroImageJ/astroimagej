 ...
 * card.
 * <li>The header package defines sets of enumerations that allow users to create
 ...
 * <li>The TableHDU class provides a large number of methods to access and modify
...
public class Fits implements Closeable {
    ...
    /**
     * <p>
     * Creates a new (empty) FITS container associated with a file input. If the
     * file is compressed a stream will be used, otherwise random access will be
     * supported.
     * </p>
     * <p>
     * While the FITS object is associated with the specified file, it is
     * initialized as an empty container with no data loaded from the input
     * automatically. You may want to call {@link #read()} to load all data from
     * the input and/or {@link #readHDU()}/{@link #getHDU(int)} for select HDUs,
     * which you can then add via {@link #addHDU(BasicHDU)} to the container.
     * </p>
     * 
     * @param myFile The File object. The content of this file will not be read
    ...
    /**
     * <p>
     * Creates a new (empty) FITS container associated with an input that
     * supports generalized random access.
     * </p>
     * <p>
    ...
    /**
     * <p>
     * Creates a new (empty) FITS container associated with {@link FitsFile}
     * input.
     * </p>
     * <p>
     * While the FITS object is associated with the specified file input, it is
     * initialized as an empty container with no data loaded from the input
     * automatically. You may want to call {@link #read()} to load all data from
     * the input and/or {@link #readHDU()}/{@link #getHDU(int)} for select HDUs,
     * which you can then add via {@link #addHDU(BasicHDU)} to the container.
     * </p>
     *
     * @param src the random access input. The content of this input will not be
     *            read into the Fits object until the user makes some explicit
     *            request.
     *
     * @throws FitsException if the input could not bew repositions to its
     *             beginning
     *
     * @see #Fits(File, boolean)
     * @see #Fits(RandomAccessFileIO)
    ...
    /**
     * <p>
     * Creates a new (empty) FITS container associated with the given input
     * stream. Compression is determined from the first few bytes of the stream.
     * </p>
     * <p>
     * While the FITS object is associated with the specified input stream, it is
     * initialized as an empty container with no data loaded from the input
     * automatically. You may want to call {@link #read()} to load all data from
     * the input and/or {@link #readHDU()}/{@link #getHDU(int)} for select HDUs,
     * which you can then add via {@link #addHDU(BasicHDU)} to the container.
     * </p>
     * 
     * @param str The data stream. The content of this stream will not be read
     *            into the Fits object until the user makes some explicit
     *            request.
     * 
     * @throws FitsException if the operation failed
    ...
    /**
     * <p>
     * Creates a new (empty) FITS container with a file or URL as its input. The
     * string is assumed to be a URL if it begins one of the protocol strings. If
     * the string ends in .gz it is assumed that the data is in a compressed
     ...
     * </p>
     * 
     * @param filename The name of the file or URL to be processed. The content
     *            of this file will not be read into the Fits object until the
     *            user makes some explicit request.
     * 
     * @throws FitsException Thrown if unable to find or open a file or URL from
     *             the string given.
     * 
     * @see #Fits(String, boolean)
     * @see #Fits(File)
    ...
    /**
     * <p>
     * Creates a new (empty) FITS container associated with a file or URL as its
     * input. The string is assumed to be a URL if it begins one of the protocol
     ...
     * </p>
     * <p>
     * While the FITS object is associated with the resource, it is initialized
     * as an empty container with no data loaded from the input automatically.
     * You may want to call {@link #read()} to load all data from the input
     * and/or {@link #readHDU()}/{@link #getHDU(int)} for select HDUs, which you
     * can then add via {@link #addHDU(BasicHDU)} to the container.
     * </p>
     * 
     * @param myURL The URL to be read. The content of this URL will not be read
     *            into the Fits object until the user makes some explicit
     *            request.
     * 
     * @throws FitsException Thrown if unable to find or open a file or URL from
    ...
    /**
     * <p>
     * Creates a new (empty) FITS container associated with a given uncompressed
     * URL as its input.
     * </p>
     * <p>
     * While the FITS object is associated with the resource, it is initialized
     ...
     * </p>
     * 
     * @param myURL The URL to be associated with the FITS file. The content of
    ...
    /**
     * Add an HDU to the Fits object. Users may intermix calls to functions which
     * read HDUs from an associated input stream with the addHDU and insertHDU
     * calls, but should be careful to understand the consequences.
     * 
     ...
     * 
     * @param n The index of the HDU to be deleted. If n is 0 and there is more
     *            than one HDU present, then the next HDU will be converted from
     *            an image to primary HDU if possible. If not a dummy header HDU
    ...
    /**
     * Checks if the value of the EXTNAME keyword of the specified HDU matches
     * the specified name.
     * 
     * @param hdu The HDU whose EXTNAME to check
     * @param name The expected name
     * 
     * @return <code>true</code> if the HDU has an EXTNAME keyword whose value
     *             matches the specified name (case sensitive!), otherwise
     *             <code>false</code>
     * 
     * @see #getHDU(String)
     */
    ...
    /**
     * Checks if the value of the EXTNAME and EXTVER keywords of the specified
     * HDU match the specified name and version.
     * 
     * @param hdu The HDU whose EXTNAME to check
     * @param name The expected name
     ...
     * 
     * @return <code>true</code> if the HDU has an EXTNAME keyword whose value
     *             matches the specified name (case sensitive!) AND has an EXTVER
     *             keyword whose value matches the specified integer version. In
    ...
    /**
     * Returns the HDU by the given extension name (defined by EXTNAME). This
     * method checks only for EXTNAME but will ignore the version (defined by
     * EXTVER). If multiple HDUs have the same matching EXTNAME, this method will
     * return the first match only.
     * 
     * @param name The name of the HDU as defined by EXTNAME (case sensitive)
     * 
     * @return The first HDU that matches the specified extension name and
     *             version, or <code>null</code> if the FITS does not contain a
     *             matching HDU.
     * 
     * @throws FitsException if the header could not be read
     * @throws IOException if the underlying buffer threw an error
    ...
    /**
     * Returns the HDU by the given extension name and version (defined by
     * EXTNAME and EXTVER keywords). If multiple HDUs have the same matching name
     * and version, this method will return the first match only.
     * 
     * @param name The name of the HDU as defined by EXTNAME (case sensitive)
     * @param version The extension version as defined by EXTVER in the matching
     *            HDU.
     * 
     * @return The first HDU that matches the specified extension name and
     *             version, or <code>null</code> if the FITS does not contain a
     ...
     *             after opening a Fits object when they wish detailed control
     *             for writing some part of the FITS file.
     */
     ...
     * 
     * @return an array of all HDUs in the Fits object. Returns null if there are
     *             no HDUs associated with this object.
     * 
    ...
    /**
     * <p>
     * Computes the <code>CHECKSUM</code> and <code>DATASUM</code> values for the
     * specified HDU index and stores them in the HUS's header. For deferred data
     * the data sum is calculated directly from the file (if possible), without
     ...
     * 
     * @throws FitsException if there was a problem computing the checksum for
     *             the HDU
    ...
    /**
     * <p>
     * Add or modify the CHECKSUM keyword in all headers. As of 1.17 the checksum
     * for deferred data is calculated directly from the file (if possible),
     * without loading the entire (potentially huge) data into RAM for the
     * calculation.
     * </p>
     * <p>
     * As of 1.17, the routine calculates checksums both for HDUs that are in
     * RAM, as well as HDUs that were not yet loaded from the input (if any). Any
     * HDUs not in RAM at the time of the call will stay in deferred mode (if the
     * HDU itself supports it). After setting (new) checksums, you may want to
     * call #rewrite()
     * </p>
     * 
     * @throws FitsException if there was an error during the checksumming
     *             operation
     * @throws IOException if there was an I/O error while accessing the data
     *             from the input
     * 
    ...
    /**
     * Calculates the data checksum for a given HDU in the Fits. If the HDU does
     * not currently have data loaded from disk (in deferred read mode), the
     * method will calculate the checksum directly from disk. Otherwise, it will
    ...
    /**
     * Calculates the FITS checksum for a given HDU in the Fits. If the HDU does
     * not currently have data loaded from disk (i.e. in deferred read mode), the
     * method will compute the checksum directly from disk. Otherwise, it will
     * calculate the checksum from the data in memory.
     * 
     * @param hduIndex The index of the HDU for which to calculate the HDU
     *            checksum
     * 
    ...
    /**
     * @deprecated This method is poorly conceived as we cannot really read FITS
     *                 from just any <code>ArrayDataInput</code> but only those,
     *                 which utilize {@link nom.tam.util.FitsDecoder} to convert
     *                 Java types to FITS binary format, such as
     *                 {@link FitsInputStream} or {@link FitsFile} (or else a
     *                 wrapped <code>DataInputStream</code>). As such, this
     *                 method is inherently unsafe as it can be used to parse
     *                 FITS content iscorrectly. It will be removed from the
     *                 public API in a future major release. Set the data stream
     *                 to be used for future input.
     * 
     ...
     *                 {@link #getNumberOfHDUs()} instead. Note size() will read
     *                 the input file/stream to the EOF before returning the
     *                 number of HDUs which {@link #getNumberOfHDUs()} does not.
     *                 If you wish to duplicate this behavior and ensure that the
     *                 input has been exhausted before getting the number of HDUs
     *                 then use the sequence: <code>
     *    read(); 
     *    getNumberOfHDUs();
     * </code>
     * 
     ...
     * 
     * @throws FitsException if there were any errors writing the contents
     *             themselves.
    ...
    /**
     * Writes the contents to a designated FITS output stream. It is up to the
     * caller to close the stream as appropriate after writing to it.
     * 
     ...
     * @throws FitsException if there were any errors writing the contents
     *             themselves.
     * @throws IOException if the underlying output stream could not be created
     *             or closed.
     * 
     * @see #write(FitsOutputStream)
     */
    ...
    /**
     * Re-writes all HDUs that have been loaded (and possibly modified) to the
     * disk, if possible -- or else does nothing. For HDUs that are in deferred
     * mode (data unloaded and unchanged), only the header is re-written to disk.
     * Otherwise, both header and data is re-written. Of course, rewriting is
     * possible only if the sizes of all headers and data segments remain the
     * same as before.
     *
     * @throws FitsException If one or more of the HDUs cannot be re-written, or
     *             if there was some other error serializing the HDUs to disk.
    ...
    /**
     * Writes the contents to the specified file. It simply wraps
     * {@link #write(File)} for convenience.
     * 
     * @param fileName the file name/path
     * 
     * @throws FitsException if there were any errors writing the contents
     *             themselves.
     * @throws IOException if the underlying stream could not be created or
     *             closed.
     * 
     * @since 1.16
     * 
    ...
    /**
     * @deprecated This method is poorly conceived as we cannot really write FITS
     *                 to just any <code>DataOutput</code> but only to specific
     *                 {@link ArrayDataOutput}, which utilize
     *                 {@link nom.tam.util.FitsEncoder} to convert Java types to
     *                 FITS binary format, such as {@link FitsOutputStream} or
     *                 {@link FitsFile} (or else a wrapped
     *                 <code>DataOutputStream</code>). As such, this method is
     *                 inherently unsafe as it can be used to create unreadable
     *                 FITS files. It will be removed from a future major
     *                 release. Use one of the more appropriate other
     *                 <code>write()</code> methods instead. Writes the contents
     *                 to an external file or stream. The file or stream remains
     *                 open and it is up to the caller to close it as
     *                 appropriate.
     * 
    ...
    /**
     * Checks if the file ends before the current read positon, and if so, it may
     * log a warning. This may happen with {@link FitsFile} where the contract of
     * {@link RandomAccess} allows for skipping ahead beyond the end of file,
     * since expanding the file is allowed when writing. Only a subsequent read
     * call would fail.
     *
     * @param in the input from which the FITS content was read.
     * 
     * @return <code>true</code> if the current read position is beyond the
     *             end-of-file, otherwise <code>false</code>.
     * 
