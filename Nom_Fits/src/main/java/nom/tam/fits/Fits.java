 ...
 * <p>
 * This FITS package attempts to make using FITS files easy, but does not do exhaustive error checking. Users should not
 * assume that just because a FITS file can be read and written that it is necessarily legal FITS. These classes try to
 * make it easy to transform between arrays of Java primitives and their FITS encodings.
 * <ul>
 * <li>The Fits class provides capabilities to read and write data at the HDU level, and to add and delete HDU's from
 * the current Fits object. A large number of constructors are provided which allow users to associate the Fits object
 * with some form of external data. This external data may be in a compressed format.
 * <p>
 * Note that this association is limited, it only specifies where the various read methods should read data from. It
 * does not automatically read the data content and store the results. To ensure that the external content has been read
 * and parsed the user may wish to invoke the read() method after creating the Fits object associated with external
 * data. E.g.,
 * 
 * <pre>
 *     File fl = ...  ; 
 ...
 * </pre>
 * 
 * Users can choose to read only some of the HDUs in a given input, and may add HDU's that were either read from other
 * files or generated by the program. See the various read and addHDU methods.
 * <li>The FitsFactory class is a factory class which is used to create HDUs. HDU's can be of a number of types derived
 * from the abstract class BasicHDU. The hierarchy of HDUs is:
 * <ul>
 * <li>BasicHDU
 * <ul>
 * <li>ImageHDU
 ...
 * </ul>
 * </ul>
 * <li>The Header class provides many functions to add, delete and read header keywords in a variety of formats.
 * <li>The HeaderCard class provides access to the structure of a FITS header card.
 * <li>The header package defines sets of enumerations that allow users to create and access header keywords in a
 * controlled way.
 * <li>The Data class is an abstract class which provides the basic methods for reading and writing FITS data. It
 * provides methods to get the the actual underlying arrays and detailed methods for manipulation specific to the
 * different data types.
 * <li>The TableHDU class provides a large number of methods to access and modify information in tables.
