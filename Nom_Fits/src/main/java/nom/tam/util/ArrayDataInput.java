...
public interface ArrayDataInput extends InputReader, DataInput, FitsIO {
     ...
     * {@link InputReader#read(byte[], int, int)}, for the full length of the
    ...
    /**
     * Read an array of booleans, possibly including legal <code>null</code>
     * values.
     * 
     * @return number of bytes read.
     * 
    ...
    /**
     * Reads into an array of booleans, possibly including legal
     * <code>null</code> values. The method has a default implementation, calls
     * {@link #readBoolean()} element by element. Classes that implement this
     * interface might want to replace that with a more efficient block read
     * implementation and/or to add the desired translation for <code>null</code>
     * values.
     * 
     * @return number of bytes read.
     * 
    ...
    /**
     * Reads a Java array from the input, translating it from its binary
     * representation to Java data format. The argument may be a generic Java
     * array, including multi-dimensional arrays and heterogeneous arrays of
     * arrays. The implementation may not populate the supplied object fully. The
     * caller may use the return value to check against the expected number of
     * bytes to determine whether or not the argument was fully poupulated or
     * not.
     * 
     * @param o a Java array object, including heterogeneous arrays of arrays. If
     *            <code>null</code>, nothing will be read from the output.
    ...
    /**
     * Reads a Java array from the input, populating all elements, or else
     * throwing and {@link java.io.EOFException}.
     * 
     * @param o a Java array object, including heterogeneous arrays of arrays. If
     *            <code>null</code>, nothing will be read from the output.
     * 
     * @throws IOException if there was an IO error, including and
     *             {@link java.io.EOFException}, while reading from the input
     * @throws IllegalArgumentException if the supplied object is not a Java
     *             array or if it contains Java types that are not supported by
     *             the decoder.
     * 
     ...
     * (However that isn't supposed to happen with a random access file, so there
     * is probably no operational difference).
     * 
     * @param distance the number of bytes to skip. Negative arguments are
     *            generally allowed, and subclass implementations may support it
     *            or else return 0 when a negative distance is specified.
     * 
    ...
    /**
     * Skips a number of bytes from the input. This differs from the
     * {@link #skip(long)} method in that it will throw an EOF if the skip cannot
     * be fully accomplished as requested...
     * 
     * @param toSkip the number of bytes to skip forward. Subclass
     *            implementations may support negative valued arguments for a
     *            backward skip also.
     * 
     * @throws EOFException if the end (or beginning) of the stream was reached
     *             before skipping the required number of bytes. This does not
     *             happen typically with forward skips on random access files,
     *             where positioning beyond the EOF is generally allowed for
     *             writing.
    ...
    /**
     * @deprecated This call is handled by {@link #skipAllBytes(long)}, without
     *                 the need for a separate implementation. Skips a number of
     *                 bytes from the input. See {@link #skipAllBytes(long)}.
     * 
     * @param toSkip the number of bytes to skip forward. Subclass
     *            implementations may support negative valued arguments for a
     *            backward skip also.
