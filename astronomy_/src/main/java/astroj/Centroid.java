...
//	public boolean forgiving = false;
	 ...
	 */
	public boolean measure(ImagePlus imp, double xx, double yy, double rr, double r1, double r2,
                           boolean findCentroid, boolean useBackgroundPlane, boolean removeStars)
		{
	...
	public boolean getBackground(ImageProcessor ip)
        ...
        if (rBack2 <= rBack1) return true; //no background region, return with background = 0
                ...
                for (int j = j1; j <= j2; j++)   // REMOVE STARS FROM BACKGROUND
                    ...
                    for (int i=i1; i <= i2; i++)
                        ...
                        if (rad2 >= r12 && rad2 <= r22)
                            ...
                            if (!Float.isNaN(val) && (iteration == 0 || (val <= backMean + 2.0*backstdev && val >= backMean - 2.0*backstdev)))
                                {
                                back += val;
                                back2 += val * val;
                                backCount++;
			...
			for (int i=i1; i<=i2; i++)
                ...
                if (rad2 >= r12 && rad2 <= r22)
                    ...
                    if (!removeBackStars || (val <= backMean + 2.0*backstdev && val >= backMean - 2.0*backstdev))	// 2011-MAR-24 CORRECTED TO CIRCULAR REGION
                        ...
                        back += val;
                        backCount++;
                        if (usePlaneLocal) plane.addPoint (di,dj,val);
                        }
                    }
				}
			}
   ...
   public float getSourceMax(ImageProcessor ip, double xCenter, double yCenter, double radius)
		...
		for (int j=j1; j<=j2; j++)
			...
			for (int i=i1; i<=i2; i++)
                ...
                if (rad2 <= rr2)	// 2011-MAR-24 CORRECTED TO CIRCULAR REGION
                    {
				    val = ip.getPixelValue(i,j);
				    if (val > sourceMax) sourceMax=val;
                    }
				}
			}    
        return sourceMax;
	...
	public boolean measure(ImageProcessor ip, Rectangle rct)
		...
		int iteration = 100;
        if (!getBackground(ip)) return false;
		if (!reposition) iteration=0;
		while (stillMoving && iteration > 0)  //iterate and find centroid within aperture 
            ...
            if (!useHowellCentroidMethod)
                {      // centroid using method written by F.V. Hessman, Georg-August-Universitat Gottingen, source unknown
                ...
                for (int j=j1; j <= j2; j++)
                    ...
                    for (int i=i1; i <= i2; i++)
                        ...
                        if (rad2 <= radius2)	// 2011-MAR-24 CORRECTED TO CIRCULAR REGION
                            ...
                            if (!Float.isNaN(val))
                                {
                                if (usePlaneLocal) back = plane.valueAt (di,dj);                                
...
//                    IJ.log("Centroid ERROR : no signal in centroid using rectangle i1,j1=" + i1 + "," + j1 + ", i2,j2=" + i2 + "," + j2);
                ...
                {   // Centroid using method based on Howell, CCD Astronomy, 2nd Ed., p. 105
                    // Code written and contributed by Matthew Craig, Minnesota State University Moorhead
//                IJ.log("Howell centroid method");
                ...
                for (int j = j1; j <= j2; j++) 
                    ...
                    for (int i = i1; i <= i2; i++) 
                        ...
                        if (!Float.isNaN(val))
                            {
                            if (usePlaneLocal) back = plane.valueAt ((double)i-xCenter+Centroid.PIXELCENTER,(double)j-yCenter+Centroid.PIXELCENTER);                            
                            I_total += val - back;
                            num++;
                            }
                        }
                ...
                for (int i = i1; i <= i2; i++) 
                    ...
                    for (int j = j1; j <= j2; j++) 
                        ...
                        if (!Float.isNaN(val))
                            {
                            if (usePlaneLocal) back = plane.valueAt(di,(double)j-yCenter+Centroid.PIXELCENTER);
                            I_i += val - back;
                            }
                        }
                    dval = I_i - I_bar;
                ...
                for (int j = j1; j <= j2; j++) 
                    ...
                    for (int i = i1; i <= i2; i++) 
                        ...
                        if (!Float.isNaN(val))
                            {   
                            if (usePlaneLocal) back = plane.valueAt ((double)i-xCenter+Centroid.PIXELCENTER, dj);
                            J_j += val - back;
                            }
                        }
                    dval = J_j - J_bar;
...
//                    IJ.log("Centroid ERROR : no signal in centroid using rectangle i1,j1=" + i1 + "," + j1 + ", i2,j2=" + i2 + "," + j2);
		...
		if (wgt != 0.0)
			...
			if (num > 1) variance = variance*srcmax*srcmax/(double)(num-1);
			}
		else
            {
...
//            }
	...
	public double background() { return backMean; }
    public double peak() { return srcmax; }
