...
public class GenericSwingDialog extends JDialog implements ActionListener, TextListener, FocusListener, ItemListener,
    ...
    public <T extends Enum<T> & RadioEnum> Map<T, JRadioButton> addRadioOptions(Class<T> tClass, Consumer<T> consumer, T def, boolean addToGui) {
        var c = getConstraints();
        var x = getXPos();
        ...
        AtomicReference<T> val = new AtomicReference<>(def);
        for (T value : values) {
